...
def test_create_user(test_client):           # pytest -k "test_create_user"   - проверка этой функции
    ...
    response = test_client.post("/user_app/users/", json=user_data)  # Этот проходит
...
def test_get_users(test_client, session):  # test_client: фикстура, возвращающая тестовый клиент (TestClient), который используется для отправки запросов к API.
    ...
    test_client.post("/user_app/users/", json=user_data_1)  #  Отправляет POST-запрос
    test_client.post("/user_app/users/", json=user_data_2)

    # Проверяем количество пользователей
    response = test_client.get("/user_app/users/") # Отправляет GET-запрос к маршруту /users/, чтобы получить список всех пользователей из базы данных через API.
...
def test_update_user(test_client):   # pytest -k "test_update_user"
    ...
    create_response = test_client.post("/user_app/users/", json=user_data)
    ...
    response = test_client.put(f"/user_app/users/{user_id}", json=updated_user_data)
...
def test_delete(test_client):  # pytest -k "test_delete"
    ...
    delete_response = test_client.delete(f"/user_app/users/{user_id}")
